...
# Define your directory in Google Drive
save_dir = os.getcwd()
...
class LunarLanderDQNAgent:
    def __init__(self, state_space, action_space, alpha=0.0005, eps=1.0, eps_decay=0.995, eps_min=0.01, gamma=0.99, batch_size=64, buffer_size=100000, seed=None):
        ...
        self.eps_max = eps  # Initial max epsilon
        self.eps = eps
        ...
        self.timestep = 0  # Initialize the timestep
    ...
    def update_q(self, state, action, next_state, reward, done):
        ...
        if done:
            ...
            # Sigmoid epsilon decay
            k = 0.01  # Control the steepness
            t_0 = 500  # Midpoint of the sigmoid
            self.eps = self.eps_min + (self.eps_max - self.eps_min) / (1 + np.exp(k * (self.timestep - t_0)))
